        ...
        <nav className="flex items-center justify-between px-4 py-3 overflow-x-auto" id="project-filters">
            ...
            <div className="flex items-center gap-2">
                {filters.map((filter) => (
                    ...
                    >
                        {filter === 'All' ? t.allProjects : filter}
                    </button>
                ))}
            </div>
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
                <button
                    ...
                    className={`p-2 rounded-lg transition-all duration-300 ${viewMode === 'carousel' ? 'bg-blue-500/20 border-blue-500/50' : 'hover:bg-white/10'
                        }`}
                    style={{
                ...
                >
                    <Layers
                        className={`w-4 h-4 transition-colors ${viewMode === 'carousel' ? 'text-blue-400' : 'text-white/50'
                            }`}
                    />
                </button>
                <button
                    onClick={() => onViewModeChange('grid')}
                    className={`p-2 rounded-lg transition-all duration-300 ${viewMode === 'grid' ? 'bg-blue-500/20 border-blue-500/50' : 'hover:bg-white/10'
                        }`}
                    style={{
                ...
                >
                    <LayoutGrid
                        className={`w-4 h-4 transition-colors ${viewMode === 'grid' ? 'text-blue-400' : 'text-white/50'
                            }`}
                    />
                </button>
            </div>
